...
# Lista de cartas de la lotería mexicana
cartas = [
...
@app.route('/loteria')
def loteria():
    return render_template('index.html', filas=4, columnas=4, cartas=random.sample(cartas, 16))

@app.route('/loteria/<int:x>')
def loteria_x(x):
    return render_template('index.html', filas=x, columnas=4, cartas=random.sample(cartas, 4 * x))

@app.route('/loteria/<int:x>/<int:y>')
def loteria_x_y(x, y):
    return render_template('index.html', filas=x, columnas=y, cartas=random.sample(cartas, x * y))
